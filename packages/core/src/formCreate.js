...
export default function createFormCreate(drive) {
  ...
  function setComponent(id, component) {
    ...
    } else if(id) return components[toString(id)];
  ...
  function margeGlobal(_options) {
    if(isBool(_options.sumbitBtn)) $set(_options, 'sumbitBtn', { show: _options.sumbitBtn });
    if(isBool(_options.resetBtn)) $set(_options, 'resetBtn', { show: _options.resetBtn });
  ...
  class FormCreate {
    ...
    createHandler(rules, child) {
      rules.map((_rule, index) => {
        ...
        if(_rule.__handler__) {
          ...
          if(handler.vm !== this.vm && !handler.deleted) return console.error(`第${index + 1}条规则正在其他的 <form-create> 中使用` + errMsg());
          handler.vm = this.vm;
    ...
    addHandlerWatch(handler) {
      ...
      let unWatch = vm.$watch(() => vm.cptData[field], (n) => {
        ...
        let trueValue = handler.toValue(n),
      ...
      const bind = () => {
        if(this.handlers[field] === undefined) delHandler(handler);
        else this.$tick(() => handler.render.sync());
      };
