...
export default class Handle {
    ...
    loadRule(rules, child) {
        rules.map((_rule) => {
            ...
            Object.defineProperty(parser.rule, 'value', {
                get: () => {
                    return parser.toValue(this.formData[parser.field]);
                },
                set: (val) => {
                    console.log('set parser');
                    this.obData[parser.field] = val;
                }
            });
    ...
    setParser(parser) {
        ...
        this.obData[field] = parser.rule.value;
    ...
    onInput(parser, value) {
        ...
        // parser.rule.value = parser.toValue(value);
    ...
    created() {
        ...
        vm.$set(vm, 'validate', this.validate);
    ...
    addParserWitch(parser) {
        ...
        parser.watch.push(vm.$watch(() => this.obData[field], (n) => {
            console.trace(field, 'obData change');
            this.formData[field] = parser.toFormValue(n);
            this.refresh();
        }, {deep: true}));

        //
        //
        // let unWatch = vm.$watch(() => this.formData[field], (n) => {
