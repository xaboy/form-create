...
export default class Handle {
    ...
    getComponent(rule) {
        ...
        if (this.components[rule.field])
            ...
            return new Parser(rule, id, true);
    }
    ...
    onInput(parser, value) {
        ...
        parser.watchFormValue && parser.watchFormValue(value);
    }
    ...
    addParserWitch(parser) {
        ...
        let unWatch = vm.$watch(() => vm.cptData[field], (n) => {
            ...
            if (vm._change(field, json)) {
                this.setValue(parser, trueValue);
                parser.watchFormValue && parser.watchFormValue(n);
            }
        }, {deep: true});
        let unWatch2 = vm.$watch(() => vm.trueData[field].value, (n) => {
            ...
            if (vm._change(field, json)) {
                ...
                parser.watchValue && parser.watchValue(n);
            }
        }, {deep: true});
    ...
    $emit(parser, eventName, ...params) {
        ...
        else if (parser.noValue === true && parser.el.$emit) this.el.$emit(eventName, ...params);
    }
