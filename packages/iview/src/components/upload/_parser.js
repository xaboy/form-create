...
import {$set, extend, isUndef, toString} from '@form-create/utils';
...
export default class Parser extends BaseParser {
    ...
    changeParseValue(parseValue) {
        ...
        this.rule.value = this.toValue(parseValue);
        this.vm._changeValue(this.field,this.toValue(parseValue));
        this._render.h.refresh();
    }
    ...
    initProps() {
        ...
        this.propsData = this.vData.props(this.uploadOptions)
            ...
            .ref(this.refName).key(`fip${this.unique}`);
    }
    ...
    onSuccess(response, file, fileList) {
        ...
        this.changeParseValue(fileList);
    }
    ...
    render(children, render) {
        ...
        vn.push(this.makeUploadBtn(unique, isShow));
        vn.push(...children);
        ...
        return this.vNode.icon({
            key: `upri${key}${index}`, props: {type: 'ios-trash-outline'}, nativeOn: {
                'click': () => {
                    ...
                    this.el.handleRemove(this.el.fileList[index]);
                    // let fileList = this.el.fileList, file = fileList[index];
                    // fileList.splice(index, 1);
